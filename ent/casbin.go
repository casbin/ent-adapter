...
func (a *Adapter) LoadPolicy(model model.Model) error {
	policies, err := a.client.CasbinRule.Query().Order(Asc("id")).All(a.ctx)
	if err != nil {
		return err
	}
...
func (a *Adapter) WithTx(fn func(tx *Tx) error) error {
...
func (a *Adapter) savePolicyLine(tx *Tx, ptype string, rule []string) *CasbinRuleCreate {
...
func (a *Adapter) createPolicies(tx *Tx, ptype string, policies [][]string) error {
	lines := make([]*CasbinRuleCreate, 0)
	for _, policy := range policies {
		lines = append(lines, a.savePolicyLine(tx, ptype, policy))
	}
